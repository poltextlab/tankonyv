# Felügyelet nélküli tanulás - Topikmodellezés {#lda_ch}
...
```{r optclus, fig.cap="A klaszterek optimális száma"}
...
A kialakított csoportokat vizuálisan is megjeleníthetjük (ld. \@ref(fig:PM_speech). ábra).

```{r PM_speech, fig.cap="A miniszterelnöki beszédek klaszterei"}
...
```{r toptoc, tidy = TRUE, tidy.opts=list(width.cutoff=80), fig.cap="A 15 leggyakoribb token a korpuszban"}
...
Ezt egy ábrán is megjelenítjük (ld. \@ref(fig:freqtoc). ábra).

```{r freqtoc, fig.cap="A 15 leggyakoribb token a korpuszban, a bővített stop szó listával"}
...
```{r zavarossag, fig.cap="Zavarosság változása a k függvényében", cache=FALSE}
...
Ezután a `ggplot2` csomag segítségével ábrán is megjeleníthetjük (ld. \@ref(fig:Gibbs9802). és \@ref(fig:Gibbs_02_06 ábra).
...
```{r Gibbs_02_06, fig.cap="2002–2006-os ciklus topikok és kifejezések (Gibbs mintavétellel)"}
top_terms_gibbs %>%
  filter(electoral_cycle == "2002-2006") %>%
  ggplot(aes(reorder_within(term, beta, topic), beta)) +
  geom_col(show.legend = FALSE) +
