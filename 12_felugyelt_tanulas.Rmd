...
A először behívjuk a szükséges csomagokat, melyek közül a `quanteda` a szokásos szövegbányászati alapcsomagunk, az `e1071` az SVM algoritmus [^svm_algoritmus] használatát teszi a lehetővé, a `ggplot2` a vizualizációt segíti, a `dplyr` a korpuszon végzett műveletekhez kell, a `SparseM` pedig a mátrixtranszformációkhoz.

[^svm_algoritmus]: A Support Vector Machine egy felügyelt gépi tanulási algoritmus, amelyet általában osztályozási és regressziós feladatokhoz használnak.
...
Ezt követi a szövegelőkészítésen és tisztításon már átesett adatok betöltése. Az egyszerűség kedvéért a `HunMineR` csomagból.
...
Majd szétválasztjuk a két újsághoz tartozó cikkeket is (esetünkben a *Népszabadság*-ot alapul véve).
...
Végezetül előrejelzéseinket ezek kódonkénti gyakorisága szempontjából ábrázoljuk (ld. \@ref(fig:svm). ábra).
...
```{r svm, include = TRUE, fig.cap="Az SVM modell klasszifikációs eredményei"}
